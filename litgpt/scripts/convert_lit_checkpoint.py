...
def get_llama_init_hf_config() -> dict:
    return {
...
def convert_config_lit_to_hf(lit_config_dict: dict) -> dict:
    ...
    hf_config_dict = get_llama_init_hf_config()
...
def convert_lit_checkpoint(checkpoint_dir: Path, output_dir: Path, model_only: bool = False) -> None:
    ...
    with incremental_save(model_output_path) as saver:
        lit_weights = lazy_load(lit_model_pth)
    ...
    if not model_only:
        ...
        for file in tokenizer_files:
            ...
            if src.exists():
                ...
                raise FileNotFoundError(f"File {src} not found for copying tokenizer files.")
